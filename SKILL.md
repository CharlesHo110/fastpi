...
| superpowers | 公网 skill | 项目级 | 公网流程 skill 集(brainstorming/TDD/systematic-debugging/计划与评审) |
| codegraph | MCP | 用户级 | 代码图谱,语义导航/重构/诊断(非 skill) |
| rtk | CLI 工具 | 用户级 | Token 优化命令包装器(60-90%;非 skill) |
...
# 装完用 rtk init 激活(用户级,一次性;pi 是否被 rtk init 支持以 rtk 官方文档为准)
```
...
| 安全门控(权限/危险确认) | 官方示例:permission-gate / protected-paths / confirm-destructive / dirty-repo-guard / git-checkpoint | 扩展(示例) | 拷贝到用户级扩展目录(~/.pi/agent/extensions/) |
